...
static inline int
reg_int_cb(struct int_param_s *int_param)
{
...
reg_int_cb(struct int_param_s *int_param)
{
	sensor_board_irq_connect(int_param->pin, int_param->cb,
				 int_param->arg);
	return 0;
}
 ...
 */
int
mpu_read_reg(unsigned char reg, unsigned char *data)
{
 ...
 */
int
mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
	unsigned char tmp[6];
 ...
 */
int
mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
	unsigned char tmp[6];
 ...
 */
int
mpu_get_temperature(long *data, unsigned long *timestamp)
{
	unsigned char tmp[2];
	short raw;
	...
	data[0] =
	    (long) ((35 +
		     ((raw -
		       (float) st.hw->temp_offset) / st.hw->temp_sens)) *
		    65536L);
	return 0;
}
 ...
 */
int
mpu_set_accel_bias(const long *accel_bias)
{
...
mpu_reset_fifo(void)
{
	...
	if (st.chip_cfg.dmp_on) {
		...
		if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
			return -1;
		if (i2c_write
		    (st.hw->addr, st.reg->fifo_en, 1,
		     &st.chip_cfg.fifo_enable))
			return -1;
	}
	return 0;
}
...
#ifdef AK89xx_SECONDARY
		mpu_set_compass_sample_rate(min
					    (st.chip_cfg.
					     compass_sample_rate,
...
mpu_set_compass_sample_rate(unsigned short rate)
{
#ifdef AK89xx_SECONDARY
	unsigned char div;
	if (!rate || rate > st.chip_cfg.sample_rate
	...
	if (i2c_write(st.hw->addr, st.reg->s4_ctrl, 1, &div))
		return -1;
	st.chip_cfg.compass_sample_rate =
	    st.chip_cfg.sample_rate / (div + 1);
	return 0;
#else
	return -1;
#endif
 ...
 */
int
mpu_get_power_state(unsigned char *power_on)
{
	if (st.chip_cfg.sensors)
		power_on[0] = 1;
	else
 ...
 */
int
mpu_get_int_status(short *status)
{
	unsigned char tmp[2];
	if (!st.chip_cfg.sensors)
		return -1;
 ...
 */
int
mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
...
//    log_i("FIFO count: %hd\n", fifo_count);
	...
	if ((index != packet_size)
	    && st.chip_cfg.fifo_enable & INV_XYZ_ACCEL) {
 ...
 */
int
mpu_read_fifo_stream(unsigned short length, unsigned char *data,
		     unsigned char *more)
{
	unsigned char tmp[2];
...
accel_self_test(long *bias_regular, long *bias_st)
{
	...
	for (jj = 0; jj < 3; jj++) {
		st_shift_cust =
		    labs(bias_regular[jj] - bias_st[jj]) / 65536.f;
		if (st_shift[jj]) {
...
compass_self_test(void)
{
	...
	do {
		delay_ms(10);
		if (i2c_read
		    (st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
			goto AKM_restore;
...
get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
	...
	for (ii = 0; ii < packet_count; ii++) {
		...
		if (i2c_read
		    (st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH,
		     data))
			return -1;
...
#ifdef EMPL_NO_64BIT
	gyro[0] =
	    (long) (((float) gyro[0] * 65536.f) / test.gyro_sens /
		    packet_count);
	gyro[1] =
	    (long) (((float) gyro[1] * 65536.f) / test.gyro_sens /
		    packet_count);
	gyro[2] =
	    (long) (((float) gyro[2] * 65536.f) / test.gyro_sens /
		    packet_count);
	if (has_accel) {
		accel[0] =
		    (long) (((float) accel[0] * 65536.f) /
			    test.accel_sens / packet_count);
		accel[1] =
		    (long) (((float) accel[1] * 65536.f) /
			    test.accel_sens / packet_count);
		accel[2] =
		    (long) (((float) accel[2] * 65536.f) /
			    test.accel_sens / packet_count);
...
#else
	gyro[0] =
	    (long) (((long long) gyro[0] << 16) / test.gyro_sens /
		    packet_count);
	gyro[1] =
	    (long) (((long long) gyro[1] << 16) / test.gyro_sens /
		    packet_count);
	gyro[2] =
	    (long) (((long long) gyro[2] << 16) / test.gyro_sens /
		    packet_count);
	accel[0] =
	    (long) (((long long) accel[0] << 16) / test.accel_sens /
		    packet_count);
	accel[1] =
	    (long) (((long long) accel[1] << 16) / test.accel_sens /
		    packet_count);
	accel[2] =
	    (long) (((long long) accel[2] << 16) / test.accel_sens /
		    packet_count);
	/* Don't remove gravity! */
	if (accel[2] > 0L)
		accel[2] -= 65536L;
 ...
 */
int
mpu_write_mem(unsigned short mem_addr, unsigned short length,
	      unsigned char *data)
{
	unsigned char tmp[2];
 ...
 */
int
mpu_read_mem(unsigned short mem_addr, unsigned short length,
	     unsigned char *data)
{
	unsigned char tmp[2];
 ...
 */
int
mpu_load_firmware(unsigned short length, const unsigned char *firmware,
...
#define LOAD_CHUNK  (16)
	...
	for (ii = 0; ii < length; ii += this_write) {
		...
		if (mpu_write_mem
		    (ii, this_write, (unsigned char *) &firmware[ii]))
			return -1;
 ...
 */
int
mpu_get_dmp_state(unsigned char *enabled)
{
 ...
 */
int
mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
	unsigned char tmp[9];
 ...
 */
int
mpu_get_compass_fsr(unsigned short *fsr)
{
#ifdef AK89xx_SECONDARY
 ...
 */
int
mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time,
			unsigned char lpa_freq)
{
	...
	if (lpa_freq) {
		...
		if (!st.chip_cfg.int_motion_only) {
			...
			mpu_get_sample_rate(&st.chip_cfg.cache.
					    sample_rate);
			st.chip_cfg.cache.sensors_on = st.chip_cfg.sensors;
			mpu_get_fifo_config(&st.chip_cfg.cache.
					    fifo_sensors);
		}
#ifdef MPU6050
