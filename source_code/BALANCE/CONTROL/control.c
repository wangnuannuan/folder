...
{	
    if(Motor_A<-amplitude_A) Motor_A=-amplitude_A;	//��????????��?A????????????
    if(Motor_A>amplitude_A)  Motor_A=amplitude_A;	  //��????????��?A????????????
    if(Motor_B<-amplitude_B) Motor_B=-amplitude_B;	//��????????��?B????????????
    if(Motor_B>amplitude_B)  Motor_B=amplitude_B;		//��????????��?B????????????
    if(Motor_C<-amplitude_C) Motor_C=-amplitude_C;	//��????????��?C????????????
    if(Motor_C>amplitude_C)  Motor_C=amplitude_C;		//��????????��?C????????????
    if(Motor_D<-amplitude_D) Motor_D=-amplitude_D;	//��????????��?C????????????
    if(Motor_D>amplitude_D)  Motor_D=amplitude_D;		//��????????��?C????????????
}
/**************************************************************************
????????????????��???????? 
...
void CAN_N_Usart_Control(void)
{
    ...
    if(Run_Flag==0)//?????
    {
        if(CAN_ON_Flag==1||Usart_ON_Flag==1) 
        {
            if(rxbuf[0]==1)
            {
                if((rxbuf[7]&0x04)==0)flag_1=1;  else flag_1=-1;  //???????��
                if((rxbuf[7]&0x02)==0)flag_2=1;  else flag_2=-1;  //???????��
                if((rxbuf[7]&0x01)==0)flag_3=1;  else flag_3=-1;  //???????��
            ...
            if(rxbuf[0]==2)
            {
                if((rxbuf[7]&0x08)==0)flag_1=1;  else flag_1=-1;  //???????��
                if((rxbuf[7]&0x04)==0)flag_2=1;  else flag_2=-1;  //???????��
                if((rxbuf[7]&0x02)==0)flag_3=1;  else flag_3=-1;  //???????��
                if((rxbuf[7]&0x01)==0)flag_4=1;  else flag_4=-1;  //???????��
